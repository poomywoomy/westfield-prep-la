    ...
    <section className="py-20 bg-gradient-to-b from-background to-orange-50/30">
      <div className="container mx-auto px-4">
        <motion.div ref={ref} initial={{ opacity: 0, y: 20 }} animate={isVisible ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.5 }} className="text-center mb-12">
          ...
          <p className="text-lg text-muted-foreground max-w-2xl mx-auto"><TranslatedText>Get answers to common questions about our Amazon FBA prep services.</TranslatedText></p>
        </motion.div>
        <motion.div initial={{ opacity: 0, y: 20 }} animate={isVisible ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.5, delay: 0.2 }} className="max-w-3xl mx-auto">
          <Accordion type="single" collapsible className="space-y-4">
            {faqs.map((faq, index) => (
              <AccordionItem key={index} value={`item-${index}`} className="border border-border rounded-xl px-6 bg-card hover:border-orange-200 transition-colors duration-200 data-[state=open]:border-orange-300 data-[state=open]:shadow-md">
                <AccordionTrigger className="text-left font-semibold hover:text-orange-600 hover:no-underline py-4"><TranslatedText>{faq.question}</TranslatedText></AccordionTrigger>
                <AccordionContent className="text-muted-foreground pb-4 leading-relaxed"><TranslatedText>{faq.answer}</TranslatedText></AccordionContent>
              </AccordionItem>
            ))}
          </Accordion>
        </motion.div>
