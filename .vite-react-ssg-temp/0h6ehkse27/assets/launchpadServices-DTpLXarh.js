import { ShoppingBag, Store, FileImage, Palette, Box, Camera, PenTool } from "lucide-react";
const THEMES = {
...
const RAW_SERVICES = [
  {
    ...
    description: "We build Shopify storefronts that are tuned for conversion from day one. Theme, payments, taxes, shipping zones, and a curated app stack picked for your category — not a Frankenstein of plugins.",
    includes: [
  ...
  },
  {
    ...
    description: "We set up Seller Central the right way the first time so you don't get suspended in week three. Brand Registry, ungating, parent/child variations, and account health baked in.",
    includes: [
  ...
  },
  {
    ...
    description: "Premium A+ content, comparison charts, and Amazon Storefronts designed like a brand site, not an afterthought. Built to lift conversion and AOV.",
    includes: [
  ...
  },
  {
    ...
    description: "Custom storefront design across Shopify and Amazon. Designed once, looks right on desktop, tablet, and mobile. Built around how your shoppers actually scroll.",
    includes: [
  ...
  },
  {
    ...
    description: "Photoreal product renders that look like real studio shots. No samples to ship, no studio days to book. Perfect for SKUs not yet in production or large variant catalogs.",
    includes: [
  ...
  },
  {
    ...
    description: "Coordinated sessions with models, props, and lifestyle sets shot in our Los Angeles studio. Ready for Amazon, Shopify, and ad creative.",
    includes: [
  ...
  },
  {
    ...
    description: "Titles, bullets, descriptions, and backend keywords written by operators who actually sell on these platforms. Built to rank and convert, not just fill space.",
    includes: [
...
const LAUNCHPAD_SERVICES = RAW_SERVICES.map((s) => ({
  ...s,
  theme: THEMES[s.slug]
}));
const getServiceBySlug = (slug) => LAUNCHPAD_SERVICES.find((s) => s.slug === slug);
export {
  LAUNCHPAD_SERVICES as L,
