-- Comprehensive Security Enhancements (avoiding duplicates)
...
-- 2. Add check constraints for data validation (only if they don't exist)
DO $$ 
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'check_phone_not_empty'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'check_positive_quantity'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'check_positive_unit_price'
  ) THEN
    ALTER TABLE public.billing_charges
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'check_positive_total_amount'
  ) THEN
    ALTER TABLE public.billing_charges
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'check_positive_price_per_unit'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE policyname = 'Admins can view all audit logs' AND tablename = 'audit_log'
  ) THEN
